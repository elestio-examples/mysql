...
#write backup helper
echo "docker-compose exec mysql /usr/bin/mysqldump --no-tablespaces --set-gtid-purged=ON --user=root --password=${SOFTWARE_PASSWORD} --all-databases | gzip > /opt/app-backups/DB_\$(date +%Y-%m-%d-%H.%M.%S).sql.gz" > backupDB.sh;
chmod +x backupDB.sh;
