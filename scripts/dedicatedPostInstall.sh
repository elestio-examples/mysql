...
#write cli helper
echo "docker-compose exec -T mysql mysql --host=172.17.0.1 --port=24306 --user=root --password=${SOFTWARE_PASSWORD}" > mysql-docker-cli.sh;
...
#write backup helper
echo "docker-compose exec mysql /usr/bin/mysqldump --no-tablespaces --set-gtid-purged=ON --user=root --password=${SOFTWARE_PASSWORD} --all-databases --triggers --routines --events --single-transaction | gzip > /opt/app-backups/DB_\$(date +%Y-%m-%d-%H.%M.%S).sql.gz" > backupDB.sh;
chmod +x backupDB.sh;
